 ...
 * Reorganized as folders containing shift papers underneath (accordion-style).
 * Supports filtering by "exam" search param.
 *
 * @component
 */
export default function PapersPage() {
  ...
  useEffect(() => {
    async function loadPapers() {
      ...
      // Filter by Exam parameter if specified
      ...
      // Group questions dynamically by Exam and Year, mapping their shifts
      const groups = {};
      filteredQuestions.forEach((q) => {
        ...
        groups[key].totalQuestions += 1;
        groups[key].subjects.add(q.subject);

        const shiftName = q.shift || 'Shift 1';
        if (!groups[key].shiftsMap[shiftName]) {
          groups[key].shiftsMap[shiftName] = 0;
        }
        groups[key].shiftsMap[shiftName] += 1;
      });

      // Convert Sets to arrays and format shifts list
      const paperList = Object.values(groups)
        .map((g) => {
          const shifts = Object.entries(g.shiftsMap).map(([name, count]) => ({
            name,
